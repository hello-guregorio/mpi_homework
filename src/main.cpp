...
const bfs_func bfs_functions[] = {&bfs_queue, &bfs_layer, &mpi_bfs};

string bfs_names[] = {"bfs_queue", "bfs_layer", "mpi_bfs"};
...
void run_all_mpi(graph_t *graph, int start_vertex, string filename, int my_rank,
                 int runs) {
    ...
    if (!wrong_result[i]) {
      if (my_rank == 0)
        printf("%s,%s,total=%ld,avg=%f\n", filename.c_str(),
               bfs_names[i].c_str(), runtime[i], avg_runtime);
    } else {
      if (my_rank == 0)
...
int main(int argc, char *argv[]) {
  ...
  if (METHOD == -1) {
    ...
    if (METHOD >= 0) {
      t1 = Time::now();
      int next_runtime =
          run_bfs(bfs_functions[METHOD], graph, start_vertex, expected, runs);
      MPI_Barrier(MPI_COMM_WORLD);
      t2 = Time::now();

      // double avg_runtime = runtime[i] / (range_to - range_from);
      printf("%s took %d ms on processor %d\n", bfs_names[METHOD].c_str(),
             next_runtime, my_rank);
      if (my_rank == 0 && next_runtime > 0 && num_proc > 1)
        printf("%s and synchronization totally took %ld ms on all processors\n",
               bfs_names[METHOD].c_str(),
               chrono::duration_cast<ms>(t2 - t1).count());
    }
  }
