...
int mpi_bfs(graph_t *graph, int start_vertex, int *result) {
  ...
  while (front_in_size != 0) {
    ...
    //全收集
    MPI_Allgather(&front_out_size, 1, MPI_INT, num_sends, 1, MPI_INT,
                  MPI_COMM_WORLD);
    bool send = false;
    //若需要传递消息，则真
    ...
    if (send) {
      ...
      MPI_Allgatherv(send_indices.data(), front_out_size, MPI_INT, recv_indices,
                     num_sends, displs, MPI_INT, MPI_COMM_WORLD);
      MPI_Allgatherv(depths.data(), front_out_size, MPI_INT, recv_depths,
                     num_sends, displs, MPI_INT, MPI_COMM_WORLD);
  ...
  auto time = std::chrono::duration_cast<ms>(Time::now() - start_time).count();
